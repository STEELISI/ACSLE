...
if __name__ == "__main__":
    ...
    if github_local_user_directory:
        ...
        thread_worker = threading.Thread(target=process_files_from_queue, daemon=True)
        thread_worker.start()
    ...
    while True:
        ...
        for count,line in enumerate(ttylog_lines):
            ...
            if isinput:
                ...
                if github_local_user_directory:
                    github_files_path_list = get_filenames_from_user_command(current_working_directory, input_cmd, current_line_prompt, current_home_dir)
                    if len(github_files_path_list) > 0:
